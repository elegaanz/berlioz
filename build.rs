...
fn main() {
    println!("cargo:rerun-if-changed=berlioz.ungram");
    let grammar = std::fs::read_to_string("berlioz.ungram").unwrap();
...
impl Generator {
            ...
            match rule {
                ...
                Rule::Alt(alternatives) => {
                    let enu = Adt::Enum(
                        alternatives
                            .iter()
                            .map(|rule| self.infer_name(grammar, &rule))
                            .collect(),
                    );
                    self.nodes.push(Node {
                        name: name.to_owned(),
    ...
    fn handle_rule(&mut self, grammar: &Grammar, rule: &Rule, name: Option<String>) {
        ...
        match rule {
            ...
            Rule::Node(node) => {
                ...
                current_struct
                    .unique_fields
                    .push((name.to_snake(), node_name, false));
            }
            Rule::Token(tok) => {
                let tok_name = grammar[*tok].name.clone();
                current_struct
                    .unique_fields
                    .push((name, Self::translate_token(&tok_name), true));
            }
            Rule::Seq(rules) => {
                if requested_name {
            ...
            Rule::Opt(rule) => match **rule {
                Rule::Node(node) => {
                    ...
                    current_struct
                        .optional_fields
                        .push((name.to_snake(), ty_name, false));
                }
                Rule::Token(tok) => {
                    let ty_name = grammar[tok].name.clone().to_camel();
                    match &ty_name[..] {
                        "Extern" | "Function" | "Node" | "Unsafe" | "True" | "False" => {
                            current_struct
                                .is_fields
                                .push((Self::translate_token(&name), true))
                        }
                        _ => {}
                    }
                    current_struct
                        .optional_fields
                        .push((name.to_snake(), ty_name, true));
                }
                Rule::Labeled { .. }
                | Rule::Seq(_)
                | Rule::Alt(_)
    ...
    fn write_header(&mut self) {
        ...
        writeln!(self.out).ok();
        writeln!(self.out, "use crate::parser::SyntaxNode;").ok();
        writeln!(self.out, "use crate::parser::SyntaxToken;").ok();
        writeln!(self.out, "use crate::ast::AstNode;").ok();
        writeln!(self.out, "use crate::ast::AstToken;").ok();
        writeln!(self.out, "use crate::lexer::SyntaxKind;").ok();
        ...
        for node in self.nodes {
            ...
            match node.def {
                Adt::Struct(struc) => {
                    ...
                    writeln!(self.out, "    fn can_cast(kind: SyntaxKind) -> bool {{").ok();
                    writeln!(self.out, "        kind == SyntaxKind::{}", kind).ok();
                    writeln!(self.out, "    }}").ok();
                    writeln!(self.out, "").ok();
                    writeln!(
                        self.out,
                    ...
                    writeln!(
                        self.out,
                        "        Self::cast(syntax).expect(\"Failed to cast to {}\")",
                        kind
                    )
                    .ok();
                    writeln!(self.out, "    }}").ok();
                    ...
                    if !struc.is_fields.is_empty()
                        || !struc.list_fields.is_empty()
                        || !struc.optional_fields.is_empty()
                        || !struc.unique_fields.is_empty()
                    {
                        writeln!(self.out).ok();
                        ...
                        for (unique_field, ty, is_token) in struc.unique_fields {
                            ...
                            writeln!(
                                self.out,
                                "    pub fn {}(&self) -> Option<{}> {{",
                                Self::unreserv(unique_field.to_snake()),
                                ty
                            )
                            .ok();
                            if is_token {
                                ...
                                writeln!(
                                    self.out,
                                    "        self.syntax().children().filter_map({}::cast).nth({})",
                                    ty, type_count
                                )
                                .ok();
                            }
                ...
                Adt::Enum(variants) => {
                    ...
                    writeln!(
                        ...
                        variants
                            .iter()
                            .map(|name| format!("{}::can_cast(kind)", name))
                            .collect::<Vec<_>>()
                            .join(" || ")
                    )
                    .ok();
                    ...
                    for other_var in vars {
                        writeln!(
                            self.out,
                            "            .or_else(|| {}::cast(syntax.clone()).map(Self::{}))",
                            other_var, other_var
                        )
                        .ok();
                    }
    ...
    fn unreserv(name: String) -> String {
        match &name[..] {
            "extern" | "unsafe" | "const" | "type" | "true" | "false" | "enum" | "struct"
            | "impl" | "if" | "else" | "mod" => format!("r#{}", name),
            _ => name,
        }
    }
}
